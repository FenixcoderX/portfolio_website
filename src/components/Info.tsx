...
const Info = ({ info }: { info: InfoData }) => {
  ...
  return (
    <section id="info" className="flex sm:items-center  h-[calc(100vh-5rem)] relative">
      {/* Info container */}
      <div className="mt-10 px-2 w-full sm:w-2/3 slide-up-fade-in">
        {/* Main info */}
        ...
        <p className="text-color-1  text-lg leading-6 min-[380px]:text-xl sm:text-2xl font-medium  tracking-normal mb-4 min-[380px]:mb-5  ">{info.hello} <br></br> {info.description}</p>
        {/* Links */}
      ...
      <button onClick={() => scrollTo('skills')}>
        <FaAnglesDown className=" text-color-3 absolute bottom-24 sm:bottom-5 left-1/2 transform -translate-x-1/2 w-10 h-10"></FaAnglesDown>
      </button>
      {/* Text on background */}
      ...
      {/* Image on the right */}
      <img src="assets/img/info-image1.svg" alt="developer" className="absolute bottom-32 sm:bottom-auto right-[27%] sm:right-0 w-[45%] sm:w-1/3 reveal " />
    </section>
  );
};
