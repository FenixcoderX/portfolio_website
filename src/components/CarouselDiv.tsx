...
export default function Carousel({ images }: { images: images }) {
  ...
  return (
    ...
    <div {...handlers} className="carousel-container">
      ...
      <div className="overflow-hidden relative pb-8 my-8">
        {/* Container that holds all the images and moves them based on the value of current variable */}
        <div
          className={`flex transition ease-out duration-500`}
        >
          {images.map((img, index) => {
            return <div className={`transition-opacity duration-500 ${current!==index ? 'hidden ' :'opacity-100'}`} key={index} ><img src={img} alt={'img' + (index + 1)} key={index} className='object-contain'/></div>;
          })}
        </div>

        {/* Buttons to move to the previous and next images */}
        <div className="top-0 flex justify-end items-end h-full w-full text-white text-3xl   ">
        ...
        <div className="absolute bottom-0 py-4 flex justify-start gap-3 w-full">
          {images.map((_, index) => {
            return (
              <div
                onClick={() => {
