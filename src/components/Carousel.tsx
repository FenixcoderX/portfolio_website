...
export default function Carousel({ images }: { images: Images }) {
  ...
  return (
   ...
   <div {...handlers} className="carousel-container">
      ...
      <div className="overflow-hidden relative pb-8 my-8 border-green-500 border-2">
        {/* Container that holds all the images and moves them based on the value of current variable */}
        <div
          className={` transition ease-out duration-500 Carousel_DisplayFrame__untpB`}
          style={{
            transform: `translateX(-${current * 100}%)`,
          }}
        >
          {images.map((img, index) => {
            return <div className="Carousel_CarouselItem__bXmqw"><img src={img} alt={'img' + (index + 1)} key={index} className='object-contain'/></div>;
          })}
        </div>
